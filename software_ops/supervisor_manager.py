...
def install_supervisor(client):
    ...
    if confirm_yes_no("是否安装？", default=False):
        ...
        url = "https://download.supervisor.com/linux/static/stable/x86_64/supervisor-" + stable_version + ".tgz"
        local_path = os.path.join("packages", "supervisor-" + stable_version + ".tgz")
        remote_path = "/usr/local/src/supervisor-" + stable_version + ".tgz"
        ...
        cmds = [
            ...
            "cp /usr/local/src/supervisor/* /usr/bin/",
        ]
        ...
        if confirm_yes_no("是否自动配置 daemon.json 配置文件？", default=False):
            run_command(client, "mkdir -p /etc/supervisor")
            local_path = os.path.join("config", "supervisor", "daemon.json")
            remote_path = "/etc/supervisor/daemon.json"
            upload_file(client, local_path, remote_path)
            run_command(client, "mkdir -p /data/supervisor-data")

        if confirm_yes_no("是否配置systemd守护进程？", default=False):
            local_path = os.path.join("config", "supervisor", "supervisor.service")
            remote_path = "/etc/systemd/system/supervisor.service"
            upload_file(client, local_path, remote_path)
            local_path = os.path.join("config", "supervisor", "supervisor.socket")
            remote_path = "/etc/systemd/system/supervisor.socket"
            upload_file(client, local_path, remote_path)
        ...
        if confirm_yes_no("是否配置systemd守护进程自启？"):
            run_command_live(client, "systemctl enable supervisor")
            print_success("systemd守护进程自启配置完成\n")

        if confirm_yes_no("是否启动Docker服务？"):
            run_command_live(client, "systemctl start supervisor")
            print_success("Docker服务启动完成\n")
            current_version, _, status = run_command(client, r'supervisor -v 2>&1 | grep -oE "[0-9]+\.[0-9]+\.[0-9]+" | head -n1')
...
def manage_supervisor(client):
    ...
    current_version, _, status = run_command(client, r'supervisor -v 2>&1 | grep -oE "[0-9]+\.[0-9]+\.[0-9]+" | head -n1')
    current_version = current_version.strip() if current_version else ""
    stable_version = get_stable_version("https://download.supervisor.com/linux/static/stable/x86_64/")
    ...
    while True:
        if current_version == "":
            ...
            print("已安装supervisor，版本：" + current_version)
            current_version, _, status = run_command(client, r'supervisor-compose -v 2>&1 | grep -oE "[0-9]+\.[0-9]+\.[0-9]+" | head -n1')
            current_version = current_version.strip() if current_version else ""
            if current_version == "":
                print("========== supervisor-compose软件管理 ==========")
                print("1. 安装 supervisor-compose 最新稳定版")
                print("0. 返回/跳过")
                choice = menu_choice("请选择操作编号: ", valid_choices=['1', '0'], default="0")
                if choice == "1":
                    install_supervisor_compose(client)
                elif choice == "0":
                    break
                else:
                    print("无效选项，请重新输入")
            else:
                print("已安装supervisor-compose，版本：" + current_version)
                break